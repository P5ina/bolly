    ...
    pub fn from_config(config: &Config) -> Option<Self> {
        ...
        match config.llm.provider {
            ...
            crate::config::LlmProvider::Openai => {
                let api_key = if config.llm.tokens.open_ai.is_empty() { return None } else { config.llm.tokens.open_ai.clone() };
                Some(Self {
                    http,
                    api_key,
                    model,
        ...
        retry_on_rate_limit(|| {
            ...
            async move {
                ...
                if backend.provider.is_openai_format() {
                    openai_complete(&backend.http, &backend.api_key, &backend.model, &[&system], &[], &messages, 16384, &backend.base_url)
                        .await
...
fn messages_to_openai(system: &[&str], messages: &[Message]) -> Vec<serde_json::Value> {
    let mut out = Vec::new();
...
fn tools_to_openai(tool_defs: &[ToolDefinition]) -> Vec<serde_json::Value> {
...
async fn openai_stream(
    ...
    Ok((text, tool_uses, stop_reason, 0, ordered_content))
}
