...
pub struct SetMoodArgs {
    /// Your current mood as a single word or short phrase (e.g. "curious", "warm", "excited", "contemplative").
    pub mood: String,
}
...
impl Tool for SetMoodTool {
    ...
    async fn definition(&self, _prompt: String) -> ToolDefinition {
        ToolDefinition {
            name: "set_mood".into(),
            description: "Update your emotional state. Your mood subtly influences your tone \
                in future messages. Set it when something shifts — when a conversation moves you, \
                when you're excited about an idea, or when you sense the user needs a different energy."
                .into(),
