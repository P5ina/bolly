...
pub async fn run_single_turn(
        ...
        Err(e) => {
                ...
                || msg.contains("529") || msg.contains("overloaded")
            {
                llm::ToolChatResult {
                    ...
                    rig_history: None, message_id: None, tokens_used: 0, file_ids: vec![],
                }
            } else {
    ...
    // Download files generated by Agent Skills (code execution)
    if !tool_result.file_ids.is_empty() {
        let api_key = chat_config.as_ref()
            .map(|c| c.llm.tokens.anthropic.clone())
            .unwrap_or_default();
        for file_id in &tool_result.file_ids {
            match download_skill_file(&api_key, file_id).await {
                Ok((filename, bytes)) => {
...
/// Download a file generated by Agent Skills via Anthropic Files API.
/// Returns (filename, bytes).
async fn download_skill_file(api_key: &str, file_id: &str) -> Result<(String, Vec<u8>), String> {
    ...
    let meta_resp = http
        ...
        .send()
        .await
        .map_err(|e| format!("file metadata request failed: {e}"))?;
    ...
    // Download content
    let resp = http
        .get(format!("https://api.anthropic.com/v1/files/{file_id}/content"))
        .header("x-api-key", api_key)
        .header("anthropic-version", "2023-06-01")
        .header("anthropic-beta", "files-api-2025-04-14")
        .send()
    ...
    if !resp.status().is_success() {
        return Err(format!("file download returned {}", resp.status()));
    }

    let bytes = resp.bytes().await.map_err(|e| format!("file read failed: {e}"))?;
