	...
	<div class="auth-container">
		...
		{#if data.expired}
			<div class="auth-card">
				<div class="auth-message">
					<p class="auth-text">This verification link has expired or is invalid.</p>
					<p class="auth-hint">Request a new one below.</p>
				</div>
				<form method="POST" action="?/resend" use:enhance={() => { loading = true; return async ({ update }) => { loading = false; await update(); }; }} class="auth-form">
					{#if form?.message}
		...
		{:else if form?.resent}
			<div class="auth-success">
				...
				<p class="auth-text">Verification email sent!</p>
				<p class="auth-hint">Check your inbox and spam folder.</p>
			</div>
		{:else}
			<div class="auth-success">
				<div class="auth-success-icon">
				...
				<p class="auth-text">Check your email for a verification link.</p>
				<p class="auth-hint">Didn't get it? Check spam or resend below.</p>
			</div>
...
<style>
	...
	.auth-message { text-align: center; margin-bottom: 1.5rem; }
	...
	.auth-hint { font-size: 0.75rem; color: oklch(0.90 0.02 75 / 25%); }
