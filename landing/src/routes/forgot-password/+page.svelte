	...
	<div class="auth-container">
		...
		{#if form?.sent}
			<div class="auth-success">
				...
				<p class="auth-text">Check your email for a reset link.</p>
				<p class="auth-hint">Didn't get it? Check spam or try again in a minute.</p>
			</div>
		{:else}
			...
			<div class="auth-card">
				<form method="POST" use:enhance={() => { loading = true; return async ({ update }) => { loading = false; await update(); }; }} class="auth-form">
					<div>
						<input name="email" type="email" placeholder="Email" class="auth-input" />
					</div>
...
<style>
	...
	.auth-desc { text-align: center; font-size: 0.875rem; color: oklch(0.90 0.02 75 / 55%); margin-bottom: 1.5rem; }
