<script lang="ts">
	...
	import { ExternalLink, AlertTriangle, Loader, Mail, CreditCard, CalendarClock, XCircle, RotateCw, RefreshCw, Share2, Check, Unplug } from 'lucide-svelte';
...
<div class="min-h-dvh bg-bg">
	...
	<div class="mx-auto max-w-[900px] px-6 py-12">
		...
		<!-- Google connection -->
		<div class="mb-6 p-5 rounded-xl border" style="background: var(--color-bg); border-color: var(--color-border);">
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-3">
					<div class="w-9 h-9 rounded-lg flex items-center justify-center"
						style="background: oklch(0.5 0 0 / 8%); border: 1px solid var(--color-border);">
					...
					{#if data.googleAccount}
						<div>
							<div class="text-sm text-text">Google connected</div>
							<div class="text-xs text-text-ghost">{data.googleAccount.email} — Gmail, Calendar, Drive</div>
						</div>
					{:else}
						<div>
							<div class="text-sm text-text">Google not connected</div>
							<div class="text-xs text-text-ghost">Connect to enable Gmail, Calendar, and Drive</div>
						</div>
					{/if}
				</div>
				{#if data.googleAccount}
					<form method="POST" action="/dashboard/disconnect-google">
						<button
							type="submit"
				...
				{:else}
					<a
						...
						class="inline-flex items-center gap-1.5 text-xs py-2 px-4 rounded-lg text-warm transition-all duration-300"
						style="background: var(--color-warm-glow); border: 1px solid var(--color-border-warm);"
					>
						Connect Google
					</a>
